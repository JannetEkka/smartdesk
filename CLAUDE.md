...
| File | What It Does |
|------|-------------|
| `smartdesk_agent/__init__.py` | Package init: `from . import agent` |
| `smartdesk_agent/agent.py` | All agent definitions, callbacks, root_agent entry point |
| `smartdesk_agent/tools.py` | MCP toolsets + AlloyDB tools + OAuth tools |
| `smartdesk_agent/mcp_servers/auth.py` | Shared OAuth 2.0 helper (generate_auth_url, exchange_auth_code, is_logged_in, logout) |
| `smartdesk_agent/mcp_servers/gmail_server.py` | Gmail MCP server (list, search, read, draft emails) |
| `smartdesk_agent/mcp_servers/calendar_server.py` | Calendar MCP server (list, search, create events, find free time) |
| `smartdesk_agent/.env` | Environment config (not committed) |
